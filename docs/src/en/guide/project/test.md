...
- **Do not import `vitest` in non-test files**: `vitest` should only be imported in test files (e.g., `*.test.ts`, `*.spec.ts`, or files in `__tests__` directories). Importing it in production code or configuration files may cause `getWorkerState` errors or build failures.
